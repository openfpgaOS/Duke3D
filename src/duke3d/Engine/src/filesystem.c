...
/* Safe unaligned memory access helpers for RISC-V */
...
int32_t initgroupfile(const char  *filename)
{
    ...
    for(i=0;i<archive->numFiles;i++){
        ...
        k = read_le32(&archive->gfilelist[i][12]); // get size (safe unaligned read)
...
int32_t compress(uint8_t  *lzwinbuf, int32_t uncompleng, uint8_t  *lzwoutbuf)
{
	...
	/* Allocate work buffers on first use */
	if (lzwbuf1 == NULL) {
		lzwbuf1 = (uint8_t *)malloc(LZWSIZE+(LZWSIZE>>4));
		lzwbuf2 = (short *)malloc((LZWSIZE+(LZWSIZE>>4))*2);
		lzwbuf3 = (short *)malloc((LZWSIZE+(LZWSIZE>>4))*2);
		lzwbuf4 = (uint8_t *)malloc(LZWSIZE);
		lzwbuf5 = (uint8_t *)malloc(LZWSIZE+(LZWSIZE>>4));
	}
	...
	do
	{
		...
		or_le32(&lzwoutbuf[bitcnt>>3], addr<<(bitcnt&7));
		bitcnt += numbits;
	...
	write_le16(&lzwoutbuf[0], (int16_t)uncompleng);
	if (((bitcnt+7)>>3) < uncompleng)
	{
		write_le16(&lzwoutbuf[2], (int16_t)addrcnt);
		return((bitcnt+7)>>3);
	}
	write_le16(&lzwoutbuf[2], 0);
...
int32_t uncompress(uint8_t  *lzwinbuf, int32_t compleng, uint8_t  *lzwoutbuf)
{
	...
	strtot = (int32_t)read_le16(&lzwinbuf[2]);
	if (strtot == 0)
	{
		copybuf((void *)((lzwinbuf)+4),(void *)((lzwoutbuf)),((compleng-4)+3)>>2);
		return((int32_t)read_le16(&lzwinbuf[0])); /* uncompleng */
	}
	...
	do
	{
		dat = ((read_le32(&lzwinbuf[bitcnt>>3])>>(bitcnt&7)) & (oneupnumbits-1));
		bitcnt += numbits;
...
void kdfread(void *buffer, size_t dasizeof, size_t count, int32_t fil)
{
	...
	kread(fil,&leng,2); kread(fil,lbuf5,(int32_t )leng);
	k = 0;
	kgoal = uncompress(lbuf5,leng,lbuf4);
	...
	for(i=1;i<count;i++)
	{
		...
		{
			kread(fil,&leng,2); kread(fil,lbuf5,(int32_t )leng);
			k = 0; kgoal = uncompress(lbuf5,(int32_t )leng,lbuf4);
		}
		for(j=0;j<dasizeof;j++) ptr[j+dasizeof] = (uint8_t ) ((ptr[j]+lbuf4[j+k])&255);
		k += dasizeof;
		ptr += dasizeof;
	}
...
void dfread(void *buffer, size_t dasizeof, size_t count, FILE *fil)
{
	...
	fread(&leng,2,1,fil);
    fread(lbuf5,(int32_t )leng,1,fil);
	...
	for(i=1;i<count;i++)
	{
		if (k >= kgoal)
		{
			fread(&leng,2,1,fil); fread(lbuf5,(int32_t )leng,1,fil);
			k = 0; kgoal = uncompress(lbuf5,(int32_t )leng,lbuf4);
		}
		for(j=0;j<dasizeof;j++) ptr[j+dasizeof] = (uint8_t ) ((ptr[j]+lbuf4[j+k])&255);
		k += dasizeof;
...
void dfwrite(void *buffer, size_t dasizeof, size_t count, FILE *fil)
{
	...
	{
		leng = (short)compress(lbuf4,k,lbuf5); k = 0;
		fwrite(&leng,2,1,fil); fwrite(lbuf5,(int32_t )leng,1,fil);
	}

	for(i=1;i<count;i++)
	{
		for(j=0;j<dasizeof;j++) lbuf4[j+k] = (uint8_t ) ((ptr[j+dasizeof]-ptr[j])&255);
		k += dasizeof;
		if (k > LZWSIZE-dasizeof)
		{
			leng = (short)compress(lbuf4,k,lbuf5); k = 0;
