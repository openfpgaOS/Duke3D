...
playbackstatus MV_GetNextVOCBlock
   ...
   while( !done )
      ...
      blocktype = ( int )*ptr;
      blocklength = (ptr[1] | (ptr[2]<<8) | (ptr[3]<<16)) & 0x00ffffff;
      ptr += 4;
      ...
      switch( blocktype )
         ...
         case 9 :
            ...
            samplespeed = ptr[0] | (ptr[1]<<8) | (ptr[2]<<16) | (ptr[3]<<24);
            BitsPerSample = ( unsigned )*( ptr + 4 );
            Channels = ( unsigned )*( ptr + 5 );
            Format = ( unsigned )(ptr[6] | (ptr[7] << 8));
...
int MV_PlayLoopedVOC
   ...
   voice->NextBlock   = ptr + (ptr[0x14] | (ptr[0x15] << 8));
   voice->DemandFeed  = NULL;
   voice->LoopStart   = NULL;
   voice->LoopCount   = 0;
   voice->BlockLength = 0;
