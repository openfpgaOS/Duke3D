...
int loadpheader(uint8_t  spot,int32 *vn,int32 *ln,int32 *psk,int32 *nump)
{
    ...
    save_dfread(nump, sizeof(int32), 1, fil);
    save_dfread(tempbuf, 19, 1, fil);
    save_dfread(vn, sizeof(int32), 1, fil);
    save_dfread(ln, sizeof(int32), 1, fil);
    save_dfread(psk, sizeof(int32), 1, fil);
...
int loadplayer(int8_t spot)
{
    ...
    save_dfread(&nump, sizeof(nump), 1, fil);
    if (nump != numplayers)
    {
        save_fclose(fil);
        if (ud.recstat != 2)
...
#define LOAD_READ kdfread
    ...
    LOAD_READ(&numsectors, 2, 1, fil);
    LOAD_READ(&sector[0], sizeof(sectortype), MAXSECTORS, fil);
    LOAD_READ(&sprite[0], sizeof(spritetype), MAXSPRITES, fil);
    LOAD_READ(&headspritesect[0], 2, MAXSECTORS + 1, fil);
    LOAD_READ(&prevspritesect[0], 2, MAXSPRITES, fil);
    LOAD_READ(&nextspritesect[0], 2, MAXSPRITES, fil);
    LOAD_READ(&headspritestat[0], 2, MAXSTATUS + 1, fil);
    LOAD_READ(&prevspritestat[0], 2, MAXSPRITES, fil);
    LOAD_READ(&nextspritestat[0], 2, MAXSPRITES, fil);
    LOAD_READ(&numcyclers, sizeof(numcyclers), 1, fil);
    LOAD_READ(&cyclers[0][0], 12, MAXCYCLERS, fil);
    LOAD_READ(ps, sizeof(ps), 1, fil);
    LOAD_READ(po, sizeof(po), 1, fil);
    LOAD_READ(&numanimwalls, sizeof(numanimwalls), 1, fil);
    LOAD_READ(&animwall, sizeof(animwall), 1, fil);
    LOAD_READ(&msx[0], sizeof(int32_t), sizeof(msx) / sizeof(int32_t), fil);
    LOAD_READ(&msy[0], sizeof(int32_t), sizeof(msy) / sizeof(int32_t), fil);
    LOAD_READ((short*)&spriteqloc, sizeof(short), 1, fil);
    LOAD_READ((short*)&spriteqamount, sizeof(short), 1, fil);
    LOAD_READ((short*)&spriteq[0], sizeof(short), spriteqamount, fil);
    LOAD_READ(&mirrorcnt, sizeof(short), 1, fil);
    ...
    LOAD_READ(&clouds[0], sizeof(short) << 7, 1, fil);
    ...
    LOAD_READ(&pskybits, sizeof(pskybits), 1, fil);
    LOAD_READ(&pskyoff[0], sizeof(pskyoff[0]), MAXPSKYTILES, fil);

    LOAD_READ(&animatecnt, sizeof(animatecnt), 1, fil);
    LOAD_READ(&animatesect[0], 2, MAXANIMATES, fil);
    LOAD_READ(&ptrbuf[0], 4, MAXANIMATES, fil);
    for (i = animatecnt - 1; i >= 0; i--)
    {
        animateptr[i] = (int32_t*)((intptr_t)&sector[0] + ptrbuf[i]);
    }
    LOAD_READ(&animategoal[0], 4, MAXANIMATES, fil);
    LOAD_READ(&animatevel[0], 4, MAXANIMATES, fil);

    LOAD_READ(&earthquaketime, sizeof(earthquaketime), 1, fil);
    LOAD_READ(&ud.from_bonus, sizeof(ud.from_bonus), 1, fil);
    LOAD_READ(&ud.secretlevel, sizeof(ud.secretlevel), 1, fil);
    LOAD_READ(&ud.respawn_monsters, sizeof(ud.respawn_monsters), 1, fil);
    ud.m_respawn_monsters = ud.respawn_monsters;
    LOAD_READ(&ud.respawn_items, sizeof(ud.respawn_items), 1, fil);
    ud.m_respawn_items = ud.respawn_items;
    LOAD_READ(&ud.respawn_inventory, sizeof(ud.respawn_inventory), 1, fil);
    ud.m_respawn_inventory = ud.respawn_inventory;

    LOAD_READ(&ud.god, sizeof(ud.god), 1, fil);
    LOAD_READ(&ud.auto_run, sizeof(ud.auto_run), 1, fil);
    LOAD_READ(&ud.crosshair, sizeof(ud.crosshair), 1, fil);
    LOAD_READ(&ud.monsters_off, sizeof(ud.monsters_off), 1, fil);
    ud.m_monsters_off = ud.monsters_off;
    LOAD_READ(&ud.last_level, sizeof(ud.last_level), 1, fil);
    LOAD_READ(&ud.eog, sizeof(ud.eog), 1, fil);

    LOAD_READ(&ud.coop, sizeof(ud.coop), 1, fil);
    ud.m_coop = ud.coop;
    LOAD_READ(&ud.marker, sizeof(ud.marker), 1, fil);
    ud.m_marker = ud.marker;
    LOAD_READ(&ud.ffire, sizeof(ud.ffire), 1, fil);
    ud.m_ffire = ud.ffire;

    LOAD_READ(&camsprite, sizeof(camsprite), 1, fil);
    LOAD_READ(&connecthead, sizeof(connecthead), 1, fil);
    LOAD_READ(connectpoint2, sizeof(connectpoint2), 1, fil);
    LOAD_READ(&numplayersprites, sizeof(numplayersprites), 1, fil);
    LOAD_READ((short*)&frags[0][0], sizeof(frags), 1, fil);

    LOAD_READ(&randomseed, sizeof(randomseed), 1, fil);
...
#define SAVE_WRITE dfwrite
    ...
    SAVE_WRITE(&bv, 4, 1, fil);
    SAVE_WRITE(&ud.multimode, sizeof(ud.multimode), 1, fil);

    SAVE_WRITE(&ud.savegame[spot][0], 19, 1, fil);
    ...
    SAVE_WRITE(&wall[0], sizeof(walltype), MAXWALLS, fil);
    SAVE_WRITE(&numsectors, 2, 1, fil);
    SAVE_WRITE(&sector[0], sizeof(sectortype), MAXSECTORS, fil);
    SAVE_WRITE(&sprite[0], sizeof(spritetype), MAXSPRITES, fil);
    SAVE_WRITE(&headspritesect[0], 2, MAXSECTORS + 1, fil);
    SAVE_WRITE(&prevspritesect[0], 2, MAXSPRITES, fil);
    SAVE_WRITE(&nextspritesect[0], 2, MAXSPRITES, fil);
    SAVE_WRITE(&headspritestat[0], 2, MAXSTATUS + 1, fil);
    SAVE_WRITE(&prevspritestat[0], 2, MAXSPRITES, fil);
    SAVE_WRITE(&nextspritestat[0], 2, MAXSPRITES, fil);
    SAVE_WRITE(&numcyclers, sizeof(numcyclers), 1, fil);
    SAVE_WRITE(&cyclers[0][0], 12, MAXCYCLERS, fil);
    SAVE_WRITE(ps, sizeof(ps), 1, fil);
    SAVE_WRITE(po, sizeof(po), 1, fil);
    SAVE_WRITE(&numanimwalls, sizeof(numanimwalls), 1, fil);
    SAVE_WRITE(&animwall, sizeof(animwall), 1, fil);
    SAVE_WRITE(&msx[0], sizeof(int32_t), sizeof(msx) / sizeof(int32_t), fil);
    SAVE_WRITE(&msy[0], sizeof(int32_t), sizeof(msy) / sizeof(int32_t), fil);
    SAVE_WRITE(&spriteqloc, sizeof(short), 1, fil);
    SAVE_WRITE(&spriteqamount, sizeof(short), 1, fil);
    SAVE_WRITE(&spriteq[0], sizeof(short), spriteqamount, fil);
    SAVE_WRITE(&mirrorcnt, sizeof(short), 1, fil);
    SAVE_WRITE(&mirrorwall[0], sizeof(short), 64, fil);
    SAVE_WRITE(&mirrorsector[0], sizeof(short), 64, fil);
    SAVE_WRITE(&show2dsector[0], sizeof(uint8_t), MAXSECTORS >> 3, fil);
    SAVE_WRITE(&actortype[0], sizeof(uint8_t), MAXTILES, fil);
    SAVE_WRITE(&boardfilename[0], sizeof(boardfilename), 1, fil);

    SAVE_WRITE(&numclouds, sizeof(numclouds), 1, fil);
    SAVE_WRITE(&clouds[0], sizeof(short) << 7, 1, fil);
    SAVE_WRITE(&cloudx[0], sizeof(short) << 7, 1, fil);
    SAVE_WRITE(&cloudy[0], sizeof(short) << 7, 1, fil);

    SAVE_WRITE(&script[0], 4, MAXSCRIPTSIZE, fil);
    ...
    SAVE_WRITE(&ptrbuf[0], 4, MAXTILES, fil);

	SAVE_WRITE(&hittype[0], sizeof(struct weaponhit), MAXSPRITES, fil);
    SAVE_WRITE(&lockclock, sizeof(lockclock), 1, fil);
    SAVE_WRITE(&pskybits, sizeof(pskybits), 1, fil);
    SAVE_WRITE(&pskyoff[0], sizeof(pskyoff[0]), MAXPSKYTILES, fil);
    SAVE_WRITE(&animatecnt, sizeof(animatecnt), 1, fil);
    SAVE_WRITE(&animatesect[0], 2, MAXANIMATES, fil);

    for (i = animatecnt - 1; i >= 0; i--) {
        ptrbuf[i] = (int32_t)((intptr_t)animateptr[i] - (intptr_t)&sector[0]);
    }

    SAVE_WRITE(&ptrbuf[0], 4, MAXANIMATES, fil);

    SAVE_WRITE(&animategoal[0], 4, MAXANIMATES, fil);
    SAVE_WRITE(&animatevel[0], 4, MAXANIMATES, fil);

    SAVE_WRITE(&earthquaketime, sizeof(earthquaketime), 1, fil);
    SAVE_WRITE(&ud.from_bonus, sizeof(ud.from_bonus), 1, fil);
    SAVE_WRITE(&ud.secretlevel, sizeof(ud.secretlevel), 1, fil);
    SAVE_WRITE(&ud.respawn_monsters, sizeof(ud.respawn_monsters), 1, fil);
    SAVE_WRITE(&ud.respawn_items, sizeof(ud.respawn_items), 1, fil);
    SAVE_WRITE(&ud.respawn_inventory, sizeof(ud.respawn_inventory), 1, fil);
    SAVE_WRITE(&ud.god, sizeof(ud.god), 1, fil);
    SAVE_WRITE(&ud.auto_run, sizeof(ud.auto_run), 1, fil);
    SAVE_WRITE(&ud.crosshair, sizeof(ud.crosshair), 1, fil);
    SAVE_WRITE(&ud.monsters_off, sizeof(ud.monsters_off), 1, fil);
    SAVE_WRITE(&ud.last_level, sizeof(ud.last_level), 1, fil);
    SAVE_WRITE(&ud.eog, sizeof(ud.eog), 1, fil);
    SAVE_WRITE(&ud.coop, sizeof(ud.coop), 1, fil);
    SAVE_WRITE(&ud.marker, sizeof(ud.marker), 1, fil);
    SAVE_WRITE(&ud.ffire, sizeof(ud.ffire), 1, fil);
    SAVE_WRITE(&camsprite, sizeof(camsprite), 1, fil);
    SAVE_WRITE(&connecthead, sizeof(connecthead), 1, fil);
    SAVE_WRITE(connectpoint2, sizeof(connectpoint2), 1, fil);
    SAVE_WRITE(&numplayersprites, sizeof(numplayersprites), 1, fil);
    SAVE_WRITE((short*)&frags[0][0], sizeof(frags), 1, fil);

    SAVE_WRITE(&randomseed, sizeof(randomseed), 1, fil);
...
void menus(void)
{
    ...
    switch(current_menu)
    {
        ...
        case 15000:
			...
			_handle_events();
            if( KB_KeyPressed(sc_Escape) || KB_KeyPressed(sc_N) || KB_KeyPressed(0x1E) || RMB)
            {
...
else
{
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            sprintf(&ud.savegame[x][0], "E%dL%d SK%d",
                                    1+ud.volume_number, 1+ud.level_number,
                                    ud.player_skill);
                            inputloc = strlen(&ud.savegame[x][0]);
#else
                            ud.savegame[x][0] = 0;
                            inputloc = 0;
#endif
		...
		case 502: // from menu 50
            ...
            x = probe(186,124,0,0);
            if(x == -1 || KB_KeyPressed(sc_N) || KB_KeyPressed(0x1E) || RMB)
            {
                KB_ClearKeyDown(sc_N);
                quittimer = 0;
