...
void readsavenames(void)
{
    ...
    for (i = 0; i < D3D_MAXSAVES; i++)
    {
        ...
        if (!save_slot_valid(i)) continue;

        if ((fil = save_fopen(i, "rb")) == NULL) continue;
        save_dfread(&dummy, 4, 1, fil);
        if (dummy != BYTEVERSION) { save_fclose(fil); continue; }
        save_dfread(&dummy, 4, 1, fil);
        save_dfread(&ud.savegame[i][0], 19, 1, fil);
        save_fclose(fil);
    }
#else
    FILE *fil;
    ...
    for (i=0;i<10;i++)
    {
        ...
        if(	dummy != BYTEVERSION	 &&
			...
			dummy != BYTEVERSION_117) { fclose(fil); continue; }
