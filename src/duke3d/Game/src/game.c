...
void animatesprites(int32_t x,int32_t y,short a,int32_t smoothratio)
{
    ...
    {                             //is the perfect time to animate sprites
        ...
        {
            if(t4<0)
                /* t4 stores an encoded script pointer here, not a timer. */
            {
...
void of_progress(int pct)
{
    ...
    bar[i++] = '[';
    for (int b = 0; b < 20; b++)
        bar[i++] = (b < filled) ? (char)0xDB : (char)0xB0;
    bar[i++] = ']';
    bar[i++] = ' ';
    if (pct >= 100) { bar[i++]='1'; bar[i++]='0'; bar[i++]='0'; }
    else if (pct >= 10) { bar[i++]='0'+pct/10; bar[i++]='0'+pct%10; }
