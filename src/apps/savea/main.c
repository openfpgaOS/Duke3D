...
/* Get vsave data size for a given vsave index */
...
int main(void) {
    ...
    /* Show per-slot save sizes (even vs odd) */
    {
    ...
    if (is_virgin()) {
        ...
        for (int i = 0; i < NUM_VSAVES; i++) {
            ...
            /* Verify data persists in PSRAM by reading back */
            uint32_t iter;
        ...
        /* Flush all slots (writes CRC footer after all vsave data) */
        /* Verify the writes */
        int verify_ok = 1;
        for (int i = 0; i < NUM_VSAVES; i++) {
