...
static void build_colormap(void) {
    /* DIAGNOSTIC: fill the entire 64x256 colormap with a single
     * known palette index (0x40 = dark gray under the grayscale
     * ramp).  If the screen turns uniform DARK gray, the palookup
     * upload is reaching the GPU and the original colormap math is
     * the only remaining problem.  If the screen stays at the
     * current medium gray, the upload isn't landing where the GPU
     * reads from and we need to chase the SDRAM address mapping. */
...
static void set_palette(void) {
    /* Pure grayscale ramp 16..255.  Diagnostic step: the multi-band
     * coloured ramps were fighting build_colormap()'s linear fade
     * (which walks every texel toward index 16, so bright texels in
     * one band end up parked in another band's range mid-distance).
     * With a single grey ramp the linear fade just darkens uniformly
     * and the maze should read as a clean grey-shaded scene.
     *
     * Once this confirms the render path is fine, the proper fix for
     * coloured surfaces is to rebuild build_colormap() so each
     * (texel, shade) cell maps to a darker palette entry of the SAME
     * hue band — i.e. Doom-style hand-crafted colormap, not a linear
     * walk through index space.
     *
     * IMPORTANT: skip 0..15 — those are the OS terminal VGA palette. */
    for (int i = 16; i < 256; i++) {
        uint8_t v = (uint8_t)i;
        of_video_palette(i, ((uint32_t)v << 16) | ((uint32_t)v << 8) | v);
    }
}
